...
func main() {
	...
	var rootCmd = &cobra.Command{
		Use:   "enbuild-cli",
		Short: "CLI to interact with Enbuild manifests",
	...
	rootCmd.Flags().StringVar(&token, "token", "", "API token (or set ENBUILD_API_TOKEN)")
	rootCmd.Flags().StringVar(&baseURL, "base-url", "", "API base URL (or set ENBUILD_BASE_URL)")
	rootCmd.Flags().BoolVar(&debug, "debug", false, "Enable debug output")

	rootCmd.Flags().StringVar(&idFlag, "id", "", "Get manifest by ID")
	rootCmd.Flags().StringVar(&vcsFlag, "vcs", "", "Filter manifests by VCS (e.g., github)")
	rootCmd.Flags().StringVar(&typeFlag, "type", "", "Filter manifests by type (e.g., terraform)")
	rootCmd.Flags().StringVar(&nameFlag, "name", "", "Search manifests by name")
